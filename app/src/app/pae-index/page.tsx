...
import cardsData from "@/data/pae-cards.json";
...
export default function PAEIndexPage() {
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 16px" }}>
        ...
        <div style={{ textAlign: "center", padding: "48px 0 32px", borderBottom: "1px solid #222" }}>
          <h1 style={{ fontSize: 32, fontWeight: 700, color: "#fff", margin: 0, letterSpacing: "0.08em" }}>
            PALDEA EVOLVED INDEX
          </h1>
          <p style={{ fontSize: 14, color: "#888", marginTop: 8, marginBottom: 0 }}>
            Scarlet &amp; Violet &middot; Paldea Evolved &middot; All {TOTAL_CARDS} Cards
          </p>
        </div>

        <Section title="What is PAE-INDEX?">
          <P>
          ...
          <P>
            ...
            entire Paldea Evolved set in a single tradeable perpetual market on
            Pokeliquid. When the set&apos;s value goes up, PAE-INDEX goes up.
            When it goes down, PAE-INDEX goes down.
          </P>
        </Section>
        ...
        <Section title="Methodology">
          <BulletList
            items={[
              `Complete basket of all ${TOTAL_CARDS} cards in the Paldea Evolved set`,
        ...
        <Section title={`All ${TOTAL_CARDS} Cards`}>
          ...
          <div style={{ fontSize: 11, color: "#666", marginBottom: 12 }}>
            Showing {filteredCards.length} of {TOTAL_CARDS} cards
          </div>
          ...
          <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
            <table style={{ width: "100%", minWidth: 400, borderCollapse: "collapse", fontSize: 12, fontFamily: "'JetBrains Mono', monospace" }}>
              <thead>
                <tr>
                  {["#", "Card", "Price"].map((h) => (
                    <th key={h} style={{ textAlign: h === "Price" ? "right" : "left", padding: "10px 10px", borderBottom: "1px solid #333", color: "#666", fontWeight: 600, fontSize: 10, textTransform: "uppercase", letterSpacing: "0.06em", whiteSpace: "nowrap", position: "sticky", top: 0, background: "#0a0a0a" }}>
                      {h}
                    </th>
                  ))}
                </tr>
    ...
    <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
      {items.map((item, i) => (
        <li key={i} style={{ fontSize: 13, color: "#999", lineHeight: 1.7, padding: "4px 0", paddingLeft: 16, position: "relative" }}>
          <span style={{ position: "absolute", left: 0, color: "#00ff41" }}>&bull;</span>
          {item}
        </li>
      ))}
    </ul>
