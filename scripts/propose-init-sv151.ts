/**
 * Create Squads v4 proposals to initialize SV151 oracle + market state.
 *
 * Creates TWO vault transactions (must be executed in order):
 *   1. init_market_oracle  — creates oracle PDA with seed price $2,526
 *   2. init_market_state   — creates market state PDA with 100k max OI
 *
 * Usage:
 *   npx ts-node scripts/propose-init-sv151.ts
 */
...
const MARKET_ID = "SV151";
const SEED_PRICE = BigInt(2_526_000_000); // $2,526 * 1e6
...
async function main() {
  ...
  console.log("  4. Add SV151 to keeper + restart");
